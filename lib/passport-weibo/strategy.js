...
function Strategy(options, verify) {
  options = options || {};
  options.authorizationURL = options.authorizationURL || 'http://api.t.sina.com.cn/oauth/authorize';
  options.tokenURL = options.tokenURL || 'http://api.t.sina.com.cn/oauth/authorize?oauth_token';
...
Strategy.prototype.userProfile = function(accessToken, callback) {
    ...
    this._oauth2.getProtectedResource(this._getuidAPI, accessToken, function(err, body, res) {
        ...
        try {
            ...
            self._oauth2.getProtectedResource(self._getProfileAPI + '?uid=' + uid, accessToken, function(err, body, res) {
                if (err) return callback(new InternalOAuthError('', err));
                
                try {
                    callback(null, self.formatProfile(body, uid));
                } catch(e) {
                    callback(e);
                }
            })
...
Strategy.prototype.formatProfile = function(raw, uid) {
    var user = JSON.parse(raw);
    user.provider = 'weibo';
    user.uid = uid;
    user.displayName = raw.screenName;
    user._raw = raw;
